...
Exercise:
    ...
    Реализуйте swap, который получает 2 человека и возвращает их в
обратном порядке. Сама функция уже
на самом деле существует. Нам просто нужно предоставить ей соответствующие типы.
Также эта функция не обязательно должна ограничиваться только
Типы людей, давайте введем его так, чтобы он работал с любыми двумя типами
указанный.
...
export function swap<T,T1>(v1 : T, v2 : T1): [T1, T]{
    return [v2, v1];
}
